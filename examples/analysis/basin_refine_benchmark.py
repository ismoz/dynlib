...
def main() -> None:
    ...
    match_width = 10
    print(
        f"{'Grid Size':<{grid_width}} | {'refine = False':>{time_width}} | "
        f"{'refine = True':>{time_width}} | {'Match %':>{match_width}}"
    )
    ...
    for size in grid_sizes:
        ...
        print(
            f"{f'{size}x{size}':<{grid_width}} | {false_time:>{time_width}.2f} | "
            f"{true_time:>{time_width}.2f} | {match_percentage:>{match_width}.1f}"
        )
