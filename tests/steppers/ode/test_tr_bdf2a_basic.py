# tests/steppers/ode/test_tr_bdf2a_basic.py
"""
Integration tests: adaptive TR-BDF2 using end-user API.
...
def test_tr_bdf2a_adaptive_accuracy():
    """
    Adaptive TR-BDF2 (bdf2a) on dx/dt = -x with default tolerances.
    ...
    # TR-BDF2 with a decent Newton + tolerances should easily satisfy this
    assert rel_error < 1e-3, f"tr_bdf2a error too large: {rel_error}"
    

def test_tr_bdf2a_vs_euler_terminal_error():
    """
    For the same nominal dt (initial dt for tr_bdf2a, fixed dt for Euler),
    tr_bdf2a should be notably more accurate at T=2.0.
    """
    T = 2.0
    dt = 0.1
    x_analytic = np.exp(-T)
    ...
    # Adaptive TR-BDF2
    sim_bdf2a = setup(DECAY_MODEL, stepper="tr-bdf2a", jit=False)
    ...
    if err_eu > 0:
        assert err_bdf2a < err_eu / 5.0, (
            f"tr-bdf2a not significantly better than Euler: "
            f"tr-bdf2a={err_bdf2a}, euler={err_eu}"
        )
